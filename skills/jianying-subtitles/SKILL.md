...
description: "Import, edit, style, translate, export, and verify JianYing captions through Rust CLI commands while preserving measured timing and evidence boundaries."
license: Apache-2.0
---

# JianYing Subtitles

字幕时间来自实测语音或明确的外部字幕，不用镜头切点或人工估时替代。
...
- 触发条件：用户需要 SRT/ASS 导入导出、字幕增改、样式、翻译或字幕校验。
- 所需 capability：`captions.import`、`captions.style`、`captions.verify`。
- 最低 CLI 版本：`1.6.0`；`captions.verify` 未达到 supported 时不得声称冷重开完成。
- 默认风险：`reversible_write`。
- 输入事实：目标草稿/隔离输出、字幕文件编码与格式、时间基准、语言、样式和安全区。
- 确认点：写入已有草稿、联网/付费翻译和覆盖导出文件分别确认。
- 成功证据：`cold_reopen`；结构导入只证明 structural。
- 禁止行为：不得生成 Python 草稿脚本，不得调用外部 headless checkout，不得手估时间、制造重叠字幕或把结构校验当播放验证。

## 工作流

1. `captions list/get` 读取现状。
2. 导入前解析 SRT/ASS，拒绝负时间、结束早于开始和非预期重叠。
3. 用 `captions import-srt` 或 `captions import-ass` 写入隔离副本。
4. 使用 `captions add/set/style/translate` 做局部修改；翻译优先使用离线 by_id/by_text 映射。
