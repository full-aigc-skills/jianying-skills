...
description: "Inspect JianYing media and drafts read-only through Rust CLI probes, structural summaries, validation, and explicit evidence-level reporting."
license: Apache-2.0
---

# JianYing Inspect

读取素材和草稿事实，不改变文件。所有后续计划都应引用本次探测结果。
...
- 触发条件：用户要查看媒体信息、草稿结构、版本、差异、引用完整性或验证等级。
- 所需 capability：`media.probe`、`project.inspect`、`project.verify`。
- 最低 CLI 版本：`1.6.0`；结果中记录实际版本和 capability 状态。
- 默认风险：`read_only`。
- 输入事实：媒体或草稿绝对路径、目标检查问题、期望证据等级和宿主平台。
- 确认点：只读检查不需要确认；冷重开、播放或原生导出是独立外部动作。
- 成功证据：`structural`；除非真实执行，否则不能提升为 cold_reopen/playback/native_export。
- 禁止行为：不得生成 Python 草稿脚本，不得调用外部 headless checkout，不得修改草稿、把文件存在当内容正确或使用计划时长冒充实测时长。

## 工作流

1. `jianying media probe <media> --json` 获取时长、尺寸、流和类型。
2. `jianying project info|version|inspect <draft> --json` 获取稳定摘要。
3. `jianying project verify <draft> --json` 检查引用、时间线和 bundle 完整性。
4. 需要比较时使用 `project diff`，明确哪些差异是预期编辑。
5. 报告事实、失败字段和当前最高证据等级。

检查清单与证据含义见 [references/workflow.md](references/workflow.md)。
只读 Job 见 [examples/minimal-job.json](examples/minimal-job.json)。
